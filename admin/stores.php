...
<input type="checkbox" name="fields[store_banner]" id="store_banner" checked disabled /> <label for="store_banner"><span></span> ' . t( 'field_store_banner', "Store Banner" ) . '</label>
...
if( $_SERVER['REQUEST_METHOD'] == 'POST' && isset( $_POST['csrf'] ) && check_csrf( $_POST['csrf'], 'stores_csrf' ) ) {
    ...
    'store_banner'          => ( isset( $_FILES['store_banner'] ) ? $_FILES['store_banner'] : array() ),
...
} else if( $_GET['type'] == 'delete_store_banner' ) {

    if( admin\actions::delete_store_banner( $_GET['id'] ) ) {
    $info->store_banner = '';
    echo '<div class="a-success">' . t( 'msg_deleted', "Deleted!" ) . '</div>';
    } else
    echo '<div class="a-error">' . t( 'msg_error', "Error!" ) . '</div>';
