...
/* ADD BLOGCATEGORY */
public static function add_blog_category( $opt = array() ) {

    global $db;

    if( !ab_to( array( 'blog_categories' => 'add' ) ) ) return false;
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "INSERT INTO " . DB_TABLE_PREFIX . "blog_categories (user, name, description, date) VALUES (?, ?, ?, NOW())" );


    $stmt->bind_param( "iss", $GLOBALS['me']->ID, $opt['name'], $opt['description']);
    $execute = $stmt->execute();
    $insert_id = $stmt->insert_id;
    $stmt->close();
...
public static function edit_blog_category( $id, $opt = array() ) {

    global $db;

    if( !ab_to( array( 'blog_categories' => 'edit' ) ) ) return false;
    ...
    $category = \query\main::blog_category_info( $id );

    $stmt = $db->stmt_init();
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "blog_categories SET name = ?, description = ? WHERE id = ?" );

    $stmt->bind_param( "ssi", $opt['name'], $opt['description'],  $id );
    $execute = $stmt->execute();
    $stmt->close();
...
/* EDIT BLOGCATEGORY URL */

public static function edit_blog_category_url( $id, $opt = array() ) {
    ...
    $stmt->prepare( "SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "blog_categories WHERE id != ? AND url_title = ?" );
    $stmt->bind_param( "is", $id, $url );
    $stmt->execute();
    $stmt->bind_result( $count );
    $stmt->fetch();
    ...
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "blog_categories SET url_title = ? WHERE id = ?" );
    $stmt->bind_param( "si", $url, $id );
    $execute = $stmt->execute();
    $stmt->close();
...
public static function delete_blog_category( $id ) {

    global $db;

    if( !ab_to( array( 'blog_categories' => 'delete' ) ) ) return false;

    $id = (array) $id;

    $stmt = $db->stmt_init();
    $stmt->prepare( "DELETE FROM " . DB_TABLE_PREFIX . "blog_categories WHERE id = ?" );

    foreach( $id as $ID ) {

        if( \query\main::category_exists( $ID ) ) {

        $slider = \query\main::blog_category_info( $ID );
        ...
        $stmt->bind_param( "i", $ID );
            ...
            do_action( 'admin_blog_category_deleted', $ID );
...
public static function add_store( $opt = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "INSERT INTO " . DB_TABLE_PREFIX . "stores (feedID, user, category, popular, physical, name, link, description, tags, image, store_banner, hours, phoneno, sellonline, visible, meta_title, meta_keywords, meta_desc, lastupdate_by, lastupdate, extra, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, NOW())" );
    ...
    $store_banner = \site\images::upload( ( !empty( $opt['import_store_banner'] ) && !empty( $opt['store_banner_url'] ) && empty( $opt['store_banner']['name'] ) ? $opt['store_banner_url'] : $opt['store_banner'] ), 'store_banner_', array( 'path' => DIR . '/', 'current' => ( !empty( $opt['store_banner_url'] ) ? $opt['store_banner_url'] : '' ) ) );
    $hours = @serialize( $opt['hours'] );
    $extra = \site\utils::array_sanitize( $opt['extra'] );
    $extra = @serialize( $extra );

    $stmt->bind_param( "iiiiissssssssiisssis", $feedID, $opt['user'], $opt['category'], $opt['popular'], $opt['type'], $opt['name'], $opt['url'], $opt['description'], $opt['tags'], $logo, $store_banner, $hours, $opt['phone'], $opt['sellonline'], $opt['publish'], $opt['meta_title'], $opt['meta_keywords'], $opt['meta_desc'], $GLOBALS['me']->ID, $extra );
...
public static function edit_store( $id, $opt = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "stores SET user = ?, category = ?, popular = ?, physical = ?, name = ?, link = ?, description = ?, tags = ?, image = ?,store_banner = ?, hours = ?, phoneno = ?, sellonline = ?, visible = ?, meta_title = ?, meta_keywords = ?, meta_desc = ?, lastupdate_by = ?, lastupdate = NOW(), extra = ? WHERE id = ?" );

    $logo = \site\images::upload( $opt['logo'], 'logo_', array( 'path' => DIR . '/', 'current' => $store->image ) );
    $store_banner = \site\images::upload( $opt['store_banner'], 'store_banner_', array( 'path' => DIR . '/', 'current' => $store->store_banner ) );
    $hours = @serialize( $opt['hours'] );
    $extra = \site\utils::array_sanitize( $opt['extra'] );
    $extra = @serialize( $extra );

    $stmt->bind_param( "iiiissssssssiisssisi", $opt['user'], $opt['category'], $opt['popular'], $opt['type'], $opt['name'], $opt['url'], $opt['description'], $opt['tags'], $logo,$store_banner, $hours, $opt['phone'], $opt['sellonline'], $opt['publish'], $opt['meta_title'], $opt['meta_keywords'], $opt['meta_desc'], $GLOBALS['me']->ID, $extra, $id );
    $execute = $stmt->execute();
...
public static function delete_store_banner( $id ) {
    ...
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "stores SET store_banner = '' WHERE id = ?" );
    $stmt->bind_param( "i", $ID );

    if( $stmt->execute() && !empty( $store->store_banner ) && !preg_match( '/^http(s)?/i', $store->store_banner ) ) {
        @unlink( DIR . '/' . $store->store_banner );
    }
...
public static function add_blog( $opt = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "INSERT INTO " . DB_TABLE_PREFIX . "blogs (title, short_description, content, category ,display_name , meta_title, meta_keywords, meta_desc, image, created) VALUES (?, ?, ?, ?, ?, ?, ?,?, ?, NOW())" );

    $image = \site\images::upload( @$_FILES['image'], 'image_', array( 'path' => DIR . '/', 'current' => '' ) );
    // echo "<pre>";print_r($stmt);die;
    $stmt->bind_param( "sssisssss", $opt['title'],$opt['short_description'],$opt['content'],$opt['category'],$opt['display_name'], $opt['meta_title'], $opt['meta_keywords'], $opt['meta_desc'],$image);
    $execute = $stmt->execute();
    $insert_id = $stmt->insert_id;
    $stmt->close();
...
public static function edit_blog( $id, $opt = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "UPDATE " . DB_TABLE_PREFIX . "blogs SET title = ?, short_description = ?, display_name = ?, category = ?, content = ?, image = ? , meta_title = ?, meta_keywords = ?, meta_desc = ? WHERE id = ?" );
    // print_r($stmt); exit;
    $stmt->bind_param( "sssisssssi",  $opt['title'], $opt['short_description'], $opt['display_name'], $opt['category'], $opt['content'], $image ,$opt['meta_title'], $opt['meta_keywords'], $opt['meta_desc'], $id );
    $execute = $stmt->execute();
    $stmt->close();
