...
public function default_links() {
    ...
    if( ( $ab = ab_to( array( 'blogs' => array( 'view', 'add','categories' ) ) ) ) && list( $blogs_view, $blogs_add, $blog_categories ) = $ab ) {
        ...
        if( $blog_categories ) {
            $nav['blogs']['subnav']['categories'] = '<a href="?route=blog_categories.php&amp;action=list">' . t( 'blog_categories_view', "Categories" ) . '</a>';
        }
    }
...
public function default_store_fields( $info = object, $csrf = '' ) {
    ...
    // store_banner
    $store_banner = '<div class="row store_banner-upload"><span>' . t( 'form_store_banner', "Store Banner" ) . ':</span>
    <div>';
    if( !empty( $info->store_banner ) ) {
        $store_banner .= '<div style="display:table;margin-bottom:2px;"><img src="' . ( $imgsrc = ( preg_match( '/^http(s)?/i', $info->store_banner ) ? $info->store_banner :  '../' . $info->store_banner ) ) . '" class="avt" alt="" style="display:table-cell;vertical-align:middle;vertical-align:middle;max-width:100px;height:50px;margin:0 20px 5px 0;" />
        <div style="display:table-cell;vertical-align:middle;margin-left:25px;">';
        $store_banner .= '<a href="' . $imgsrc . '" target="_blank" class="btn">' . t( 'view', "View" ) . '</a> <a href="' . \site\utils::update_uri( '', array( 'type' => 'delete_store_banner', 'token' => $csrf ) ) . '" class="btn" data-delete-msg="' . t( 'delete_msg', "Are you sure that you want to delete this?" ) . '">' . t( 'delete', "Delete" ) . '</a>
        </div>
        </div>';
    }
    $store_banner .= '<input type="file" name="store_banner" accept="image/*" />
    </div></div>';

    $fields['store_banner']['position'] = 11;
    $fields['store_banner']['markup'] = $store_banner;
...
public function blog_category_fields( $info = object, $csrf = '' ) {

    do_action( 'before_blog_category_fields', $info );

    global $add_blog_category_fields;

    $custom_fields  = array();
    $sections       = array();

    if( !empty( $add_blog_category_fields ) && is_array( $add_blog_category_fields) ) {
        foreach( $add_blog_category_fields as $id => $link ) {
            $custom_fields[$id]['position'] = isset( $link['position'] ) ? $link['position'] : 99;
            $extra = widgets::build_extra( $link['fields'], ( isset( $info->extra ) && is_array( $info->extra ) ? $info->extra : array() ), '', 'extra', $info );
            $custom_fields[$id]['markup'] = $extra['markup'];
            $sections = array_merge( $sections, $extra['sections'] );        }
    ...
    return array( 'fields' => self::default_blog_category_fields( $info, $csrf ) + $custom_fields, 'sections' => $sections );
}

public function default_blog_category_fields( $info = object, $csrf = '' ) {
    ...
    // Description
    $fields['description']['position'] = 2;
    $fields['description']['markup'] = '<div class="row"><span>' . t( 'form_description', "Description" ) . ':</span><div><textarea name="text" style="min-height:100px;">' . ( isset( $info->description ) ? $info->description : '' ) . '</textarea></div></div>';

    return value_with_filter( 'default_blog_category_fields', $fields );
...
public function blog_category_list_markup( $item, $links ) {

    $lists = get( 'admin-list-style' );

    $links = value_with_filter( 'admin_list_blog_category', $links, $item );

    if( isset( $lists['blog_category'] ) && ( $callback = \site\utils::check_callback( $lists['blog_category'] ) ) ) {
        return call_user_func( $callback, $item, $links );
    }
...
public function default_blog_fields( $info = object, $csrf = '' ) {
    ...
    $category = '<div class="row"><span>' . t( 'form_category', "Category " ) . ':</span>
    <div><select name="category">';
    $category .= '<option value="0"' . ( !isset( $info->category ) || $info->category === 0 ? ' selected' : '' ) . '>' . t( 'no_category_select', "No Categpry" ) . '</option>';
    foreach( \query\main::while_blog_categories( array( 'max' => 0, 'show' => 'cats' ), array( 'no_emoticons' => true, 'no_filters' => true ) ) as $cat ) $category .= '<option value="' . $cat->ID . '"' . ( ( isset( $info->category ) && $info->category === $cat->ID ) || ( !isset( $info->category )) ? ' selected' : '' ) . '>' . $cat->name . '</option>';
    $category .= '</select></div></div>';

    $fields['category']['position'] = 3;
    $fields['category']['markup'] = $category;
