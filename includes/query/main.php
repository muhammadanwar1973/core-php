...
public static function blog_category_exists( $id = 0 ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "blog_categories WHERE id = ? OR url_title = ?" );
    $stmt->bind_param( "is", $id, $id );
    $stmt->execute();
    $stmt->bind_result( $count );
    $stmt->fetch();
...
/* GET INFORMATION ABOUT BLOG CATEGORY */

public static function blog_category_info( $id = 0, $special = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT id, user, (SELECT name FROM " . DB_TABLE_PREFIX . "users WHERE id = c.user), name, description, (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "blogs WHERE category = c.id), url_title, date FROM " . DB_TABLE_PREFIX . "blog_categories c WHERE id = ? OR url_title = ?" );
    $stmt->bind_param( "is", $id, $id );
    $stmt->execute();
    $stmt->bind_result( $id, $user, $user_name, $name, $description, $blogs, $url_title, $date );
    $stmt->fetch();
    $stmt->close();

    // special
    $useem = ( isset( $special['no_emoticons'] ) && $special['no_emoticons'] ) ? false : (boolean) \query\main::get_option( 'smilies_blog_categories' );
    $usesh = ( isset( $special['no_shortcodes'] ) && $special['no_shortcodes'] ) ? false : true;
    $useec = ( isset( $special['no_escape'] ) && $special['no_escape'] ) ? false : true;
    $usefl = ( isset( $special['no_filters'] ) && $special['no_filters'] ) ? false : true;

    return (object) value_with_filter( 'blog_category_info_values', array( 'ID' => $id, 'user' => $user, 'user_name' => esc_html( $user_name ), 'name' => \site\content::title( 'blog_category_name_single', $name, $useem, $useec, $usefl ), 'description' => \site\content::content( 'blog_category_single', $description, $useem, $usesh, false, $useec, $usefl ),'blogs' => $blogs, 'date' => $date, 'url_title' => esc_html( $url_title ), 'link' => ( defined( 'SEO_LINKS' ) && SEO_LINKS ? \site\utils::make_seo_link( \query\main::get_option( 'seo_link_blog_category' ), $name, $url_title, $id, \query\main::get_option( 'extension' ) ) : $GLOBALS['siteURL'] . '?cat=' . $id ) ) );
...
public static function store_info( $store_id = 0, $special = array() ) {
    ...
    $stmt->prepare( "SELECT s.id, s.feedID, s.user, s.category, s.popular, s.physical, s.name, s.link, s.description, s.tags, s.image, s.store_banner, s.hours, s.phoneno, s.sellonline, s.visible, s.views, (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "coupons WHERE store = s.id AND visible > 0), s.url_title, s.meta_title, s.meta_keywords, s.meta_desc, s.lastupdate_by, (SELECT name FROM " . DB_TABLE_PREFIX . "users WHERE id = s.lastupdate_by), s.lastupdate, s.extra, s.date, (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "reviews WHERE store = s.id AND valid > 0), (SELECT AVG(stars) FROM " . DB_TABLE_PREFIX . "reviews WHERE store = s.id AND valid > 0), (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "products WHERE store = s.id AND visible > 0), u.name FROM " . DB_TABLE_PREFIX . "stores s LEFT JOIN " . DB_TABLE_PREFIX . "users u ON (u.id = s.user) WHERE s.id = ? OR s.url_title = ?" );
    $stmt->bind_param( "is", $store_id, $store_id );
    $stmt->execute();
    $stmt->bind_result( $id, $feed_id, $user, $cat, $popular, $physical, $name, $link, $description, $tags, $image, $store_banner, $hours, $phone, $sellonline, $visible, $views, $coupons, $url_title, $meta_title, $meta_keywords, $meta_desc, $lastupdate_by, $lastupdate_by_name, $last_update, $extra, $date, $reviews, $stars, $products, $user_name);
    $stmt->fetch();
    $stmt->close();
...
/* NUMBER OF BLOGCATEGORIES */

public static function have_blog_categories( $category = array(), $special = array() ) {

    global $db;

    $blog_categories = \site\utils::validate_user_data( $category );

    $where = array();

    /* WHERE / ORDER BY */

    if( !empty( $blog_categories['ids'] ) && strcasecmp( $blog_categories['ids'], 'all' ) != 0 ) {
        $arr = array_filter( array_map( function( $w ){
            return (int) $w;
        }, explode( ',', $blog_categories['ids'] ) ));
        if( !empty( $arr ) )
        $where[] = 'id IN(' . \site\utils::dbp( implode(',', $arr) ) . ')';
        if( !isset( $blog_categories['orderby'] ) ) {
            $orderby[] = 'field(id,' . \site\utils::dbp( implode(',', $arr) ) . ')';
        }
    }

    if( !empty( $blog_categories['search'] ) ) {
        $search = implode( '.*', explode( ' ', trim( $blog_categories['search'] ) ) );
        $where[] = 'CONCAT(name, description) REGEXP "' . \site\utils::dbp( $search ) . '"';
    }

    if( isset( $blog_categories['show'] ) ) {
        $show = array_map( 'trim', explode( ',', strtolower( $blog_categories['show'] ) ) );
        $custom_where_clause = value_with_filter( 'blog_categories_where_clause', array(
            'cats'      => 'id > 0'
        ) );
        foreach( $show as $v ) {
            if( !empty( $custom_where_clause ) && in_array( $v, array_keys( $custom_where_clause ) ) ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "blog_categories" . ( empty( $where ) ? '' : ' WHERE ' . implode( ' AND ', array_filter( $where ) ) ) );
    $stmt->execute();
    $stmt->bind_result( $count );
    $stmt->fetch();
    $stmt->close();
    ...
    $pags = array();
    $pags['results'] = $count;
    $pags['per_page'] = ( !empty( $blog_categories['per_page'] ) ? (int) $blog_categories['per_page'] : \query\main::get_option( 'items_per_page' ) );
    $pags['pages'] = ceil( $pags['results'] / $pags['per_page'] );
    $page = ( !empty( $blog_categories['page'] ) ? (int) $blog_categories['page'] : ( !empty( $_GET['page'] ) ? (int) $_GET['page'] : 1 ) );
    if( $page < 1 ) $page = 1;
    if( $page > $pags['pages'] ) $page = $pags['pages'];
    $pags['page'] =    $page;
    if( $pags['pages'] > $pags['page'] ) $pags['next_page'] = \site\utils::update_uri( '', array( 'page' => ($pags['page']+1) ) );
...
/* FETCH THE BLOGCATEGORIES */

public static function while_blog_categories( $category = array(), $special = array() ) {

    global $db;

    /** make or not seo links */
    $seo_link = defined( 'SEO_LINKS' ) && SEO_LINKS ? true : false;
    list( $seo_link_blog_category, $extension ) = array( \query\main::get_option( 'seo_link_blog_category' ), \query\main::get_option( 'extension' ) );

    $blog_categories = \site\utils::validate_user_data( $category );

    $where = $orderby = $limit = array();

    if( isset( $blog_categories['max'] ) ) {
        if( !empty( $blog_categories['max'] ) ) {
            $limit[] = $blog_categories['max'];
        }
    } else {
        $page = ( !empty( $blog_categories['page'] ) ? (int) $blog_categories['page'] : ( !empty( $_GET['page'] ) ? (int) $_GET['page'] : 1 ) );
        $per_page = ( isset( $blog_categories['per_page'] ) ? (int) $blog_categories['per_page'] : \query\main::get_option( 'items_per_page' ) );
        $offset = isset( $page ) && $page > 1 ? ( $page - 1 ) * $per_page : 0;
    ...
    if( isset( $blog_categories['orderby'] ) ) {
        $order = array_map( 'trim', explode( ',', strtolower( $blog_categories['orderby'] ) ) );
        $custom_orderby_clause = value_with_filter( 'blog_categories_orderby_clause', array(
            'rand'          => 'RAND()',
            'name'          => 'c.name',
            'name desc'     => 'c.name DESC',
            'date'          => 'c.date',
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT id, user, name, description, url_title, date, (SELECT COUNT(*) FROM " . DB_TABLE_PREFIX . "blogs WHERE category = c.id) FROM " . DB_TABLE_PREFIX . "blog_categories c" . ( empty( $where ) ? '' : ' WHERE ' . implode( ' AND ', array_filter( $where ) ) ) . ( empty( $orderby ) ? '' : ' ORDER BY ' . implode( ', ', array_filter( $orderby ) ) ) . ( empty( $limit ) ? '' : ' LIMIT ' . implode( ',', $limit ) ) );
    $stmt->execute();
    $stmt->bind_result( $id, $user, $name, $description, $url_title, $date, $blogs );

    $data = array();
    while( $stmt->fetch() ) {

        $data[] = (object) value_with_filter( 'blog_category_info_values', array( 'ID' => $id, 'user' => $user, 'name' => \site\content::title( 'blog_categories_name_list', $name, $useem, $useec, $usefl ), 'description' => \site\content::content( 'blog_categories_list', $description, $useem, $usesh, false, $useec, $usefl ), 'date' => $date, 'blogs' => $blogs,  'url_title' => esc_html( $url_title ), 'link' => ( $seo_link ? \site\utils::make_seo_link( $seo_link_blog_category, $name, $url_title, $id, $extension ) : $GLOBALS['siteURL'] . '?cat=' . $id ) ) );
...
/* GET THE BLOGCATEGORIES */

public static function group_blog_categories( $category = array(), $special = array() ) {

    $array = array();

    foreach( \query\main::while_blog_categories( $category, $special ) as $c ) {
        $array['cat_' . $c->ID]['info'] = $c;
    }
...
public static function blog_info( $id = 0, $special = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT id, title, short_description, content, category ,display_name , meta_title, meta_keywords, meta_desc, image, created FROM " . DB_TABLE_PREFIX . "blogs c WHERE id = ?" );
    $stmt->bind_param( "i", $id);
    $stmt->execute();
    $stmt->bind_result( $id, $title, $short_description, $content, $category , $display_name , $meta_title, $meta_keywords, $meta_desc , $image, $date);
    $stmt->fetch();
    $stmt->close();
...
public static function while_blogs( $blog = array(), $special = array() ) {
    ...
    $stmt = $db->stmt_init();
    $stmt->prepare( "SELECT b.id, b.title, b.short_description, b.content, b.category ,b.display_name , b.meta_title, b.meta_keywords, b.meta_desc, b.image, b.created, b.publish, bc.name FROM " . DB_TABLE_PREFIX . "blogs b LEFT JOIN " . DB_TABLE_PREFIX . "blog_categories bc ON (bc.id = b.category)" . ( empty( $where ) ? '' : ' WHERE ' . implode( ' AND ', array_filter( $where ) ) ) . ( empty( $orderby ) ? '' : ' ORDER BY ' . implode( ', ', array_filter( $orderby ) ) ) . ( empty( $limit ) ? '' : ' LIMIT ' . implode( ',', $limit ) ) );
    $stmt->execute();
    $stmt->bind_result(  $id, $title, $short_description, $content, $category , $display_name , $meta_title, $meta_keywords, $meta_desc , $image, $date,$publish, $category_name);
    ...
    while( $stmt->fetch() ) {
        ...
        $data[] = (object) value_with_filter( 'blog_info_values', array( 
            'ID' => $id, 
            'title' => \site\content::title( 'blog_title_single', $title, $useem, $useec, $usefl ), 
            'category' => \site\content::title( 'blog_category_single', $category, $useem, $useec, $usefl ), 
            'display_name' => \site\content::title( 'blog_display_name_single', $display_name, $useem, $useec, $usefl ), 
            'short_description' => \site\content::content( 'blog_short_description_single', $short_description, $useem, $useec, $usefl ), 
            'content' => \site\content::content( 'blog_content_single', $content, $useem, $usesh, false, $useec, $usefl ),
            'image'=> $image,
